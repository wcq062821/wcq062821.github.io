...
对于不搞灰黑产的我的来讲这实在不是一条应该走的路(安全圈很鄙视灰黑产啊) 果断放弃
...
[00:03.11]曲：Davor Julama/Jennifer Lee Hershman
[00:04.11]词：Davor Julama/Jennifer Lee Hershman 
这种格式的真正歌词然后把所有行合并成一行 vim 下的话可以用 J 来合并当前行和下一行
然后再把光标定位到第一个字符执行命令 :.,.s/ \[/\\n\[/
这条命令会把所有的空格[ 替换成 \n[
```
...
当时只能用瑟瑟发抖来形容了 MDZZ  
今天中午开始写这第一篇文章 发现做出来跟写出来 尼玛差别太大了  
这篇文章写了一天。。。。。。。。。。 
